...
func TestActionRequiredDerivation(t *testing.T) {
	...
	for _, c := range cases {
		msg, err := d.InsertMessageWithDeliveries("p", "a", "b", c.msgType, "s", "body", "{}", "P2", 3600, nil, nil, []string{"b"}, "")
		if err != nil {
...
func TestResponseInheritance(t *testing.T) {
	...
	open, _ := d.InsertMessageWithDeliveries("p", "a", "b", "response", "s", "body", "{}", "P2", 3600, nil, nil, []string{"b"}, "")
	...
	q, _ := d.InsertMessageWithDeliveries("p", "b", "a", "question", "q", "body", "{}", "P2", 3600, nil, nil, []string{"a"}, "")
	reply, _ := d.InsertMessageWithDeliveries("p", "a", "b", "response", "r", "body", "{}", "P2", 3600, &q.ID, nil, []string{"b"}, "")
...
func TestActionRequiredExplicitOverride(t *testing.T) {
	...
	msg, _ := d.InsertMessageWithDeliveries("p", "a", "b", "notification", "s", "body", "{}", "P2", 3600, nil, nil, []string{"b"}, "do")
...
func TestWakeCountHonorsActionRequired(t *testing.T) {
	...
	_, _ = d.InsertMessageWithDeliveries("p", "a", "n1", "notification", "s", "body", "{}", "P2", 3600, nil, nil, []string{"n1"}, "")
	if got := count("n1"); got != 0 {
		t.Fatalf("none-tagged notification must not wake, count=%d", got)
	}

	// question → counted.
	_, _ = d.InsertMessageWithDeliveries("p", "a", "n2", "question", "s", "body", "{}", "P2", 3600, nil, nil, []string{"n2"}, "")
	if got := count("n2"); got != 1 {
		t.Fatalf("question must wake, count=%d", got)
	}

	// task → counted.
	_, _ = d.InsertMessageWithDeliveries("p", "a", "n3", "task", "s", "body", "{}", "P2", 3600, nil, nil, []string{"n3"}, "")
	if got := count("n3"); got != 1 {
		t.Fatalf("task must wake, count=%d", got)
	}

	// notification with explicit do → counted (opt-in wake).
	_, _ = d.InsertMessageWithDeliveries("p", "a", "n4", "notification", "s", "body", "{}", "P2", 3600, nil, nil, []string{"n4"}, "do")
	...
	_, _ = d.InsertMessageWithDeliveries("p", "a", "n5", "notification", "s", "body", "{}", "P0", 0, nil, nil, []string{"n5"}, "none")
	...
	_, _ = d.InsertMessageWithDeliveries("p", "a", "n6", "question", "s", "body", "{}", "P2", 3600, nil, nil, []string{"n6"}, "none")
...
func TestWakeCountLegacyNullWakes(t *testing.T) {
	d := testDB(t)
	msg, _ := d.InsertMessageWithDeliveries("p", "a", "leg", "notification", "s", "body", "{}", "P2", 3600, nil, nil, []string{"leg"}, "")
