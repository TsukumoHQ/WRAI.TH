...
func TestGetAgentTasks_DispatchedExcludesInactiveStatuses(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		tsk, err := d.DispatchTask("p1", "dev", "cto", "cancelled task", "", "P2", nil, nil, TypedTicket{}, false)
		if err != nil {
	...
	activeTsk, err := d.DispatchTask("p1", "dev", "cto", "active task", "", "P2", nil, nil, TypedTicket{}, false)
	if err != nil {
...
func TestGetAgentTasks_DispatchedLimited(t *testing.T) {
	...
	_, _, _ = d.RegisterAgent("p1", "cto", "lead", "", nil, nil, true, nil, "[]", 0, RegisterOptions{})
	for i := 0; i < 30; i++ {
		_, err := d.DispatchTask("p1", "dev", "cto", "task", "", "P2", nil, nil, TypedTicket{}, false)
		if err != nil {
			t.Fatalf("dispatch: %v", err)
		}
	}
...
func TestProgressNotes_RoundTrip(t *testing.T) {
	...
	tsk, _ := d.DispatchTask("p1", "dev", "cto", "long task", "", "P2", nil, nil, TypedTicket{}, false)
