...
func dispatchClaimed(t *testing.T, d *DB, project, holder string) string {
	...
	task, err := d.DispatchTask(project, "", "dispatcher", "lease me", "", "P1", nil, nil, TypedTicket{})
	if err != nil {
...
func TestReclaimNonReclaimableStateRefused(t *testing.T) {
	...
	pend, err := d.DispatchTask(project, "", "dispatcher", "pending", "", "P1", nil, nil, TypedTicket{})
	if err != nil {
...
func TestDoubleClaimTypedConflict(t *testing.T) {
	d := testDB(t)
	const project = "p1"
	task, err := d.DispatchTask(project, "", "dispatcher", "race", "", "P1", nil, nil, TypedTicket{})
	if err != nil {
		t.Fatalf("dispatch: %v", err)
	}
