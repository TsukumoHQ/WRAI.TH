...
func TestDeadletterIdempotent(t *testing.T) {
	d := testDB(t)
	m, _, err := d.InsertMessageWithDeliveries("p1", "s", "bot-a", "notification", "s", "c", "{}", "P1", 0, nil, nil, []string{"bot-a"}, "")
	if err != nil {
		t.Fatalf("insert: %v", err)
	}
	expireMessageNow(t, d, m.ID)
...
func TestDeadletterSurvivesPurge(t *testing.T) {
	d := testDB(t)
	m, _, err := d.InsertMessageWithDeliveries("p1", "s", "bot-a", "notification", "keeps", "c", "{}", "P0", 0, nil, nil, []string{"bot-a"}, "")
	if err != nil {
		t.Fatalf("insert: %v", err)
	}
	expireMessageNow(t, d, m.ID)
