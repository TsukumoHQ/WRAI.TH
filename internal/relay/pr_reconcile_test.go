...
func TestPRReconcileCandidates(t *testing.T) {
	...
	open, _ := r.DB.DispatchTask("p1", "dev", "boss", "open pr", "", "P2", nil, nil, db.TypedTicket{})
	_, _ = r.DB.SetTaskPR(open.ID, "p1", ptr("u1"), intp(1), ptr("open"), ptr("o/r"))

	// (2) merged PR → excluded (terminal PR state).
	merged, _ := r.DB.DispatchTask("p1", "dev", "boss", "merged pr", "", "P2", nil, nil, db.TypedTicket{})
	_, _ = r.DB.SetTaskPR(merged.ID, "p1", ptr("u2"), intp(2), ptr("merged"), ptr("o/r"))

	// (3) open PR but task done → excluded (terminal task).
	doneTask, _ := r.DB.DispatchTask("p1", "dev", "boss", "done task", "", "P2", nil, nil, db.TypedTicket{})
	...
	_, _ = r.DB.DispatchTask("p1", "dev", "boss", "no pr", "", "P2", nil, nil, db.TypedTicket{})
