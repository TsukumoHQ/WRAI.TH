...
func seedLinkedTask(t *testing.T, r *Relay, number int, repo string) string {
	t.Helper()
	task, err := r.DB.DispatchTask("p1", "dev", "boss", "pr work", "", "P2", nil, nil, db.TypedTicket{}, false, nil)
	if err != nil {
...
func TestSyncMagicWordAutoLinks(t *testing.T) {
	r := testRelay(t)
	task, err := r.DB.DispatchTask("p1", "dev", "boss", "human pr", "", "P2", nil, nil, db.TypedTicket{}, false, nil)
	if err != nil {
		t.Fatalf("dispatch: %v", err)
	}
	body := "does the thing\n\nrelay-task: " + task.ID
...
func TestSyncUnlinkedIgnored(t *testing.T) {
	r := testRelay(t)
	task, _ := r.DB.DispatchTask("p1", "dev", "boss", "unrelated", "", "P2", nil, nil, db.TypedTicket{}, false, nil)
