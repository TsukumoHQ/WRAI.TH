...
func TestResolveManager_FallsBackToDispatcher(t *testing.T) {
	...
	task, err := database.DispatchTask("p1", "worker", "lead2", "blocked thing", "", "P1", nil, nil, db.TypedTicket{})
	if err != nil {
...
func TestResolveDispatcher(t *testing.T) {
	...
	task, err := database.DispatchTask("p1", "prof", "the-dispatcher", "t", "", "P2", nil, nil, db.TypedTicket{})
	if err != nil {
		t.Fatalf("dispatch: %v", err)
	}
